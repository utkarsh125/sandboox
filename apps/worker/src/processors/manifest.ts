...
//goal: to read the manifest file from disk (for now)
//build the file path safely
//utilize xml2js to convert xml string to js-object 

//Interface for Permission information
//role: tracks each permission with its risk level
...
//Interface for ExportedComponents
//role: tracks activities/services/receivers/providers that are exposed to other apps
...
//Interface for ManifestData
//role: the complete parsed output
...
export async function parseManifest(decompDir: string): Promise<ManifestData> {
...
function parsePermissions(manifest: any): PermissionInfo[] {

    //iterate over uses-permission tags
    //extract full Android permission name 
    //then classify risk
    const usesPerms = manifest["uses-permission"] || [];
    return usesPerms.map((p: any) => {
        const fullName: string = p.$?.["android:name"] || "unknown";
...
function classifyPermission(shortName: string): "dangerous" | "normal" | "signature" {

    const upper = shortName.toUpperCase();

    //simple lookup against two sets
    if (DANGEROUS_PERMISSIONS.has(upper)) return "dangerous";
    if (SIGNATURE_PERMISSIONS.has(upper)) return "signature";
    return "normal";

}

//func to parse exported components
function parsedExportedComponents(app: any): ExportedComponent[] {
    ...
    const types = [
        ...
        { key: "activity", type: "activity" as const },
        { key: "service", type: "service" as const },
        { key: "receiver", type: "receiver" as const },
        { key: "provider", type: "provider" as const },
...
function getDefaultManifest(): ManifestData {
    return {
        packageName: null,
        versionName: null,
        versionCode: null,

...
export function runSemgrep(decompDir: string): SemgrepFinding[] {
...
function parseResults(rawOutput: string, baseDir: string): SemgrepFinding[] {
...
function mapToOwasp(ruleId: string): string | null {
    const lower = ruleId.toLowerCase();
    if (
        lower.includes("crypto") ||
        lower.includes("cipher") ||
...
function extractCWE(metadata: any): string[] {


    if (!metadata) return [];

    if (Array.isArray(metadata.cwe)) return metadata.cwe;

    if (typeof metadata.cwe === "string") return [metadata.cwe];
    return [];
